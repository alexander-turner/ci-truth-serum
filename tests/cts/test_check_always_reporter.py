...
Drives the module's functions directly so every branch (decide-gate detection,
always() detection, opt-out comment, YAML-shape guards, and main()'s exit code)
...
car = load_hook("check_always_reporter.py", "check_always_reporter")
...
def test_check_file_ignores_null_trigger_config(tmp_path):
    # `pull_request:` with no mapping body → cfg is None, but the trigger key still
    # exists, so a gated workflow is still flagged.
    body = "on:\n  pull_request:\n  push:\njobs:\n  decide:\n    uses: ./.github/workflows/decide-reusable.yaml\n  work:\n    needs: decide\n    if: needs.decide.outputs.run == 'true'\n    runs-on: ubuntu-latest\n"
...
def test_locate_trigger_fallback_line_for_flow_style_yaml(tmp_path):
    ...
    # _locate_trigger falls back to line 1. The workflow is still flagged.
    body = (
