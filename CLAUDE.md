...
- **Never put a `concurrency:` lock with a static `group:` (no `github.ref`/`github.head_ref`/PR-number key) at the workflow level on a required-check workflow.** GitHub keeps one running + one pending run per group and cancels the older _pending_ run wholesale when a newer ref arrives—that run starts zero jobs, so the `always()` reporter never fires and the required check hangs at “Expected — Waiting” forever. Key the group per-ref, or move the `concurrency:` block onto the expensive _job_ so the run + reporter always execute and a superseded run goes definitively red. The `check-static-concurrency` hook enforces this; opt out with `# static-concurrency-ok` only when the workflow is deliberately never a required check.
